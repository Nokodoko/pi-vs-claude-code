...
| --------------- | ------------------------- | ---------------------------------------------------------- |
| **Bun** ≥ 1.3.2 | Runtime & package manager (required for all extensions except `coms-go`) | [bun.sh](https://bun.sh) |
| **Go** ≥ 1.23   | Build toolchain for `coms-go` (coms/coms-net transport) | [go.dev/dl](https://go.dev/dl/) |
...
# Build the coms-go binary (coms/coms-net transport)
(cd extensions/coms-go && go build -o bin/coms-go ./cmd/coms-go)
```
...
| **coms-go**             | `extensions/coms-go/shim.ts`        | Go-native replacement for `coms` + `coms-net`: single static binary, no Bun runtime required for the transport. Same eight tools, same wire protocol. Load via `pi -e extensions/coms-go/shim.ts`. Build: `(cd extensions/coms-go && go build -o bin/coms-go ./cmd/coms-go)`. See [`extensions/coms-go/WORKFLOW.md`](extensions/coms-go/WORKFLOW.md) for the full runbook: standing up a server, naming agents, multi-host setup, and connection limits. |
